...
Profile photos accept JPG, PNG, or WebP up to 2 MB. The original image bytes and reversible zoom/position crop settings are saved in the existing device-local profile preferences; no photo is uploaded to a server. CSS/SVG energy sits behind the portrait without filtering the image. All 31 existing forms have distinct cosmetic treatments in `src/data/auras.mjs`, using reusable silhouettes and lightning paths from `src/data/aura-shapes.mjs`; unknown IDs use the subtle neutral Base preset. Kaioken multipliers progressively expand and intensify from one flame layer to three. The golden forms have different silhouettes: classic flames, broad Grade 2 energy, bulky Grade 3 energy, smooth Full Power energy, jagged SSJ2 flames with forked lightning, and tall SSJ3 flames with cascading bolts. Blue Kaioken and Ultra Instinct stages also differ in layers, size, glow and particles. Differences remain visible in static mode. Reduced-motion settings automatically keep the aura still, and Edit profile also offers a saved static option. Aura selection never changes scoring or unlock rules.
...
- **Your transformations:** 31 Dragon Ball forms and techniques, including Great Ape, Kaioken, Super Saiyan, Super Saiyan Blue Evolution, Ultra Instinct, Beast and Black Frieza. These names come from Dragon Ball; the shared unlocking order is an app progression across different character branches. They advance automatically from the same power estimate and never multiply it a second time. The harder version 4 scale anchors Black Frieza to American raw record performance (Jesus Olivares’ 1,153.5 kg / 2,543 lb total); cosmic benchmarks extend beyond it. A 1,550 lb total now sits around the Super Saiyan tiers in absolute mode. Exact targets vary with lift balance and the selected comparison profile. Older character-form selections are retired on upgrade.
...
- **Visible lifting goals:** goals bring weaker lifts toward comparable strength using the selected public reference, retaining stronger proven singles. New targets round up to 2.5 lb (or 1 kg) without exceeding the frozen American raw full-power records: squat 490.5 kg, bench 325 kg, deadlift 487.5 kg. Nearby one-lift options are shown only for increases of at most 10% (or one load increment); distant milestones require progress across lifts. Milestones beyond the record limits have no invented lifting target. Tap any goal for its loads and record sources. Estimated 1RM never reduces earned targets.
...
The September 2026 public references and the user-requested, harder version 4 power calibration are frozen. They do not automatically refresh or move the goalposts. New successful singles and deliberate comparison-profile changes still recalculate your progress and lift targets. See [methodology and reproducible data build](docs/strength-data.md).
