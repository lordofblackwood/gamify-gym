...
export function estimatedOneRepMax(weight, reps) {
  if (!Number.isFinite(weight) || weight <= 0 || !Number.isInteger(reps) || reps < 2 || reps > 10)
    return null;
  const estimate = weight * (1 + reps / 30);
  return Number.isFinite(estimate) ? estimate : null;
}

export function unrealizedPotential(events, actualRecords) {
  const estimates = Object.fromEntries(Object.keys(LIFTS).map((id) => [id, null]));
  for (const event of events) {
    if (event.source !== "bulgarian" || !event.success ||
        event.outcome !== "completed" || !event.singleCompleted ||
        event.unit !== "lb" || !Object.hasOwn(estimates, event.exercise)) continue;
    const estimated = estimatedOneRepMax(event.repSet?.weight, event.repSet?.reps);
    const previous = estimates[event.exercise];
    if (estimated !== null && (!previous || estimated > previous.estimated ||
        (estimated === previous.estimated && event.date > previous.date))) {
      estimates[event.exercise] = {
        estimated, weight: event.repSet.weight, reps: event.repSet.reps,
  ...
  const lifts = Object.keys(LIFTS).map((id) => {
    ...
    const projected = actual === null ? null : Math.max(actual, evidence?.estimated ?? actual);
    return { id, name: LIFTS[id], actual, evidence, projected,
      gap: actual === null ? 0 : projected - actual };
  });
  const calibrated = lifts.every((lift) => lift.actual !== null);
  const total = calibrated ? lifts.reduce((sum, lift) => sum + lift.projected, 0) : null;
  const gap = lifts.reduce((sum, lift) => sum + lift.gap, 0);
  return {
    lifts, total, gap, calibrated,
    ...
    progression: calibrated && gap > 1e-8 ? progressionFromScore(total) : null,
  };
}
